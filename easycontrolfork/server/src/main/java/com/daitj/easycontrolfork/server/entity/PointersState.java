...
public final class PointersState {
  ...
  private final ConcurrentHashMap<Integer, Pointer> pointers =
  ...
  public synchronized Pointer newPointer(int pointerId, long now) {
    if (pointers.size() >= MAX_POINTERS) return null;
  ...
  private boolean isLocalIdAvailable(int localId) {
    for (Pointer value : pointers.values()) {
      if (value.id == localId) return false;
    }
    return true;
  }
  ...
  public synchronized int update() {
    ArrayList<Pointer> active = new ArrayList<>(pointers.values());

    active.sort(Comparator.comparingInt(pointer -> pointer.id));

    int i = 0;
    for (Pointer value : active) {
  ...
  public synchronized int getPointerIndex(int localPointerId) {
    ArrayList<Pointer> active = new ArrayList<>(pointers.values());

    active.sort(Comparator.comparingInt(pointer -> pointer.id));

    for (int i = 0; i < active.size(); i++) {
      if (active.get(i).id == localPointerId) {
        return i;
      }
    }
